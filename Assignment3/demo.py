...
class MSTSpanningTree(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.link_delay = {}  # Store link delays
        self.setup_mininet_topology()  # Set up Mininet topology
        print("Controller Initialized.")
   

    def setup_mininet_topology(self):
        # Create a Mininet instance
        net = Mininet()

        # Start the Mininet topology defined in topology.py
        net.start()

        # Manually add links to the graph
        for link in net.links:
            src, dst = link.intf1.node, link.intf2.node
            delay = link.params.get('delay', '0ms').strip('ms')

            # Add edges with weights based on delay
           # self.graph.add_edge(src.name, dst.name, weight=int(delay[:-2]))  # Convert to int
            self.link_delay[(src.name, dst.name)] = delay
            self.link_delay[(dst.name, src.name)] = delay  # Assuming symmetrical delays
            print(f"{delay} from {dst.name} to {src.name}")
        self.logger.info("Mininet topology set up:")
