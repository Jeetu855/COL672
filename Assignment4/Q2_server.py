...
# Constants
MSS = 100  # Maximum Segment Size for each packet
...
cwd=3200 # initial congestion window side
THRESHOLD=cwd/2  #threshold 
...
def send_file(server_ip, server_port, enable_fast_recovery):
    ...
    with open(FILE_PATH, "rb") as file:
        ...
        ITERATION=0  ##represent no of RTT complete 
        WINDOW_SIZE=1
        pre_window_base=-1 ##just for complete one rtt
        ...
        while True:
            ...
            #print(f"WINDOW_SIZE* MSS {WINDOW_SIZE* MSS} >= THRESHOLD {THRESHOLD}")
            if WINDOW_SIZE* MSS>=THRESHOLD:     
                WINDOW_SIZE=WINDOW_SIZE+1
                print("ADDITIVE INCREASE PHASE\n ")
                print("Send packet with window size :",WINDOW_SIZE,"in Iteartion",ITERATION,"window_base",window_base,"\n")
            else :       
            ...
            n=0    ## to count no of packet send in one iteraion 
            while  pre_window_base!=window_base and  next_sequence_number < window_base + WINDOW_SIZE * MSS and not eof:
                # Read data from file
                n=n+1
                ...
                print(f"{n} Sent packet with sequence number {next_sequence_number} at {send_time} \n")
                next_sequence_number += len(data_chunk)
            if(WINDOW_SIZE* MSS>=cwd) :
                print("WINDOW_SIZE* MSS reach cwd  ",WINDOW_SIZE)
                WINDOW_SIZE=WINDOW_SIZE//2 
                print("After WINDOW_SIZE=WINDOW_SIZE//2 -1  ",WINDOW_SIZE)
            ...
            try:
                ...
                print(f"Ack receive {ack_sequence_number} from  packet {count} at {time.time()}")
                count=count+1
                ...
                if ack_sequence_number > window_base:
                  ...
                  #  print( f"Received cumulative ACK for sequence number {ack_sequence_number } at {time.time()}")
                    ...
                    #print(  f"Received duplicate ACK for sequence number {ack_sequence_number - 1}, count={duplicate_ack_count}")
               ...
               # else:
                   ...
                   #  print(f"Received old ACK for sequence number {ack_sequence_number - 1}, ignoring")
            except socket.timeout:
